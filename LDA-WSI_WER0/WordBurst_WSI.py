...
ifile = open(sys.argv[1],"r") #reference
#ifile2 = open(sys.argv[2],"r") #input

WB = dd(lambda: dd(list)) #video[kw[list]]
...
for l in line:
    ...
    if "_" in main[0]:
        ...
        video_id = token[0]
        sentence_id = main[0].strip()
    else:
        key = main[1].split(" ",1)
        kw = key[0] #word
        kw_def = key[1].strip() #definition
        data[sentence_id][kw] = kw_def.strip()
        WB[video_id][kw].append(1)

for k in data.keys():
    print k
    for kw in data[k].keys():
        token = k.rsplit("_",1)
        vid = token[0]
        if len(WB[vid][kw]) == 1:
            print "# " + kw + " -1"
